...
if [ "$AGENT_JOBSTATUS" == "Succeeded" ]; then
  ...
  curl -X POST https://api.telegram.org/bot${BOT_TOKEN}/sendMessage -d "chat_id=$ONETEXT_CHANNEL_ID&parse_mode=MarkdownV2&text=*🚀 Build $APPCENTER_BUILD_ID Succeeded*%0A%0A*Variant* $APPCENTER_ANDROID_VARIANT%0A*File Name* $name"
